...
# generate a empty matrix for random forest's result
result_rf = np.zeros((tt, 100))

# realize random forest:
# generate 20 weak rote learners, and each learner has 90 samples
for t in range(tt):
    sample_index = random.sample(range(p), sub_p)
    sub_Q_class0 = Q_class0[:, sample_index]
    sub_Q_class1 = Q_class1[:, sample_index]

    Q = [sub_Q_class0, sub_Q_class1]
    ...
    result_rf[t] = test_predict_class

result_vote_rf = []
for i in range(100):
    vote_list = list(result_rf[:, i])
    if vote_list.count(0) > vote_list.count(1):
        result_vote_rf.append(0)
    else:
        result_vote_rf.append(1)
